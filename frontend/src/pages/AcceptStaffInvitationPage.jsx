...
export const AcceptStaffInvitationPage = () => {
  ...
  useEffect(() => {
    if (!token) {
      setInvitationError('Staff invitation token is missing.');
      setLoading(false);
      return;
    }

    organizationService.getStaffInvitationDetails(token)
      .then((data) => {
        setInvitation(data);
      })
      .catch((err) => {
        const msg = err.response?.data?.message || err.response?.data?.detail || 'This staff invitation link is invalid or has expired.';
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitting(true);
    try {
      await organizationService.acceptStaffInvitation(token, {
      ...
      if (data) {
        if (data.error?.message) {
          ...
          setSubmitError('Failed to accept staff invitation.');
        }
      } else {
      ...
      <div style={{ backgroundColor: '#FAF8F3', minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '2rem' }}>
          <div style={{ textAlign: 'center', color: '#78716C' }}>
            ...
            <p style={{ fontSize: '1rem', fontWeight: 600 }}>Validating staff invitation link...</p>
          </div>
        </div>
      </div>
    );
        ...
        <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '2.5rem 1.25rem' }}>
          ...
          >
            ...
            <p style={{ color: '#78716C', fontSize: '0.95rem', lineHeight: 1.6, marginBottom: '1.75rem' }}>
              Your staff account has been successfully created and linked.<br />
              You are now an active staff member of <strong>{invitation?.organization_name}</strong>.
            </p>
            <Link
              to="/login"
              style={{
        ...
        >
          <div style={{ textAlign: 'center', marginBottom: '1.75rem' }}>
            ...
            <p style={{ color: '#78716C', fontSize: '0.9rem', margin: 0 }}>
              You were invited to join as a Staff Member for <strong>{invitation?.email}</strong>.
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit}>
            ...
            >
              {submitting ? 'Accepting Invitation...' : 'Accept Staff Invitation & Join Team'}
            </button>
          </form>
        </div>
      </div>
